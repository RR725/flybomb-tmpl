/*
 * @Author: ecofe 
 * @Date: 2018-06-29 15:55:36 
 * @Last Modified by:   ecofe 
 * @Last Modified time: 2018-06-29 15:55:36 
 */
"use strict";
...
class ProductType extends React.Component {
	...
	selectType(value, label) {
		...
		if (type === "subnav") {
			result.map(function(data) {
				const source = String(data[searchType || "appId"]);
				if (value === source) {
					value = data.appId + "";
				}
			});
	...
	render() {
		...
		if (this.props.initData.all) {
			options.unshift(
				<Option key={0} value={String(0)}>
					{this.props.initData.all}
				</Option>
			);
		}
		let cantNull = {};
		...
		if (this.props.cantNull) {
			cantNull = this.props.cantNull(
				this.props.initData.name,
				value[0].name
			);
		} else {
			changeJson = {
		...
		return (
			<div className="select_applist">
				<Select
					...
					{...changeJson}
					size="large"
					placeholder={this.props.initData.title}
					style={{ width: this.props.initData.width + "px" }}
				>
					{options}
				</Select>
			</div>
...
exports.render= connect(state => state.common, {
	// loading,
	// loaded,
	// pagination
})(ProductType);
